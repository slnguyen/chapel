...
proc test_gerc(){
  test_cugerc_helper(complex(64));
  test_cugerc_helper(complex(128));
}
...
proc test_gerc_helper(type t){
  ...
  // Simple test
  {
    ...
    // Precompute result
    for (i,j) in R.domain do R[i, j] = alpha*X[i]*conjg(Y[j]) + A[j, i];

    select t {
      when complex(64) do {
        cu_cgerc(cublas_handle, m, n, alpha, gpu_ptr_X:c_ptr(t), gpu_ptr_Y:c_ptr(t), gpu_ptr_A:c_ptr(t), m);
      }
      when complex(128) do {
        cu_zgerc(cublas_handle, m, n, alpha, gpu_ptr_X:c_ptr(t), gpu_ptr_Y:c_ptr(t), gpu_ptr_A:c_ptr(t), m);
      }
   }
